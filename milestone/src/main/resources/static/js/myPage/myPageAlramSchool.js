...
function getList(alarmResp) {
    ...
    alarmResp.arAlarmDTO.content.forEach(alarms => {
        ...
        if (alarms.checkAlarm == true) {
            ...
            if (alarms.type == "기부금") {
                text += `<div class="items-location" style="width: 30%;">` + (alarms.type + ' ' + alarms.item + ' 원을' +`<br>`+' 전달 하였습니다') + `</div>`
            } else {
                text += `<div class="items-location" style="width: 30%;">` + (alarms.type + ' ' + alarms.item + ' 에' +`<br>`+' 봉사신청 하였습니다') + `</div>`
            }
...
function getListSend(param, callback, error) {
    ...
    $.ajax({
        url: "/myPageRest/schoolAlarm" + queryString,
        type: "get",
